...
- [ ] Step 1: Add parser support for `--foreground` and `--no-tray`, and thread both flags through the parsed namespace without changing unrelated CLI routes.
- [ ] Step 2: Create `src/asky/daemon/launcher.py` with one launch-mode resolver and one background-spawn helper. Use:
- [ ] `asky --xmpp-daemon --foreground` for headless background children.
- [ ] `asky --xmpp-daemon --xmpp-menubar-child` for tray-backed background children.
- [ ] platform-appropriate detached child kwargs, including `start_new_session=True`, `stdin=subprocess.DEVNULL`, and Windows-specific detached creation flags when available.
- [ ] Step 3: Refactor `src/asky/cli/main.py` so `--daemon` dispatch goes through the launcher abstraction instead of open-coded macOS-only spawn logic. Keep `--xmpp-menubar-child` behavior intact.
- [ ] Step 4: Preserve `-vv --daemon` as a compatibility foreground alias in this handoff, but implement it through the same launch-mode resolver so the hidden coupling is no longer scattered through `main.py`.
- [ ] Step 5: Update `src/asky/daemon/startup.py` so service-manager commands use a foreground-running command and do not self-background. Keep the macOS menubar-child registration explicit where the existing LaunchAgent path depends on it.
- [ ] Step 6: Update `scripts/watch_daemon.sh` so development auto-reload continues to restart an attached daemon process by default, using `--foreground`.
...
- [ ] Step 1: Extend the shared daemon launcher helper with a `DaemonLaunchNotice` or equivalent structured summary that includes:
- [ ] log directory path
- [ ] spawned PID for background runs
- [ ] whether tray was used
- [ ] GUI URL when `gui_server` is enabled and password-configured
- [ ] GUI warning when `gui_server` is enabled but password is missing
- [ ] Step 2: Update foreground daemon startup to print the same notice format before entering the long-running loop.
- [ ] Step 3: Pass an explicit NiceGUI storage path from `GUIServerPlugin` into `NiceGUIServer`, rooted under the plugin data dir.
- [ ] Step 4: In `NiceGUIServer._run_thread` / `_default_runner`, set `NICEGUI_STORAGE_PATH` before importing or running NiceGUI so its persistence no longer resolves from cwd.
- [ ] Step 5: Add tests that prove:
- [ ] missing password still fails secure GUI startup, but startup notice warns clearly
- [ ] configured password enables URL emission
- [ ] storage path is rooted under the plugin data dir instead of `/.nicegui`
- [ ] foreground daemon CLI path still exercises full daemon startup logic rather than a reduced XMPP-only path
...
- [ ] Step 1: Update CLI help surface and plugin flag help so `--daemon`, `--foreground`, and `--no-tray` describe the new contract plainly.
- [ ] Step 2: Update recorded CLI surface manifests and integration tests so the new public flags have ownership and the `--daemon` dispatch assertions track the new launcher boundary.
- [ ] Step 3: Update user-facing docs in existing relevant sections:
- [ ] `README.md`
- [ ] `docs/xmpp_daemon.md`
- [ ] `docs/configuration.md`
- [ ] `docs/web_admin.md`
...
- [ ] `docs/troubleshooting.md`
- [ ] `docs/development.md`
- [ ] Step 4: Update internal project docs that must reflect the shipped behavior:
- [ ] `ARCHITECTURE.md`
- [ ] `devlog/DEVLOG.md`
- [ ] `src/asky/cli/AGENTS.md`
- [ ] `src/asky/daemon/AGENTS.md`
- [ ] `src/asky/plugins/AGENTS.md`
- [ ] Step 5: Run the full suite and compare runtime against the baseline in this plan. If runtime increases materially, document why in `devlog/DEVLOG.md` and make sure no new slow test was introduced without justification.
