...
- Review Log:
  - 2026-03-16 — reviewed `8d2275a1071f574531dd2e24921d6f492851b251..d30fa7f`; outcome: `changes-requested`
  - 2026-03-16 — reviewed follow-up fix batch `d30fa7f..77899a6` and finalized full handoff `8d2275a1071f574531dd2e24921d6f492851b251..77899a6`; outcome: `approved+squashed`
...
### [x] Checkpoint 1: Normalize Core Daemon Ownership and Generic Internal Flags
...
- [x] Step 1: Remove `--daemon` from plugin-contributed CLI ownership.
- [x] Step 2: Add core parser/help ownership for `--daemon` so plugin bootstrap is no longer needed just to parse that flag.
- [x] Step 3: Delete the `--daemon -> --xmpp-daemon` translation and make daemon dispatch use a direct core `args.daemon` flag.
- [x] Step 4: Replace `--xmpp-menubar-child` with hidden `--tray-child` everywhere:
- [x] CLI parser
- [x] launcher child commands
- [x] macOS app bundle launcher
- [x] startup command builders
- [x] remote blocked-flag list
- [x] manifests and tests
- [x] Step 5: Rename the launch context enum from `MACOS_APP` to `TRAY_APP` and update all references/tests.
- [x] Step 6: Move recorded CLI ownership for `--daemon` into a core daemon surface test file and remove it from plugin-surface ownership.
...
### [x] Checkpoint 2: Add Shared Runtime Ownership and Cross-Platform Tray Backend Selection
...
- [x] Step 1: Add `tray = ["pystray>=0.19.5"]` to `pyproject.toml`.
- [x] Step 2: Introduce a shared runtime-owner helper module for live daemon ownership and mutual exclusion between tray and headless modes.
- [x] Step 3: Add a generic tray launcher module that chooses macOS `rumps` vs Linux/Windows `pystray`.
- [x] Step 4: Implement `PystrayTrayApp` using `pystray.Menu` and the existing `TrayController` data model:
- [x] plugin status rows render disabled menu items
- [x] plugin action rows render clickable menu items
- [x] tray-login item and quit item are appended by core
- [x] warning rows are rendered as disabled menu items on pystray platforms and logged once
- [x] Step 5: Update the launcher path so public `--daemon` uses generic tray availability checks instead of macOS-only `rumps` checks.
- [x] Step 6: Wire runtime-owner rules:
- [x] tray takeover stops a live headless owner before the tray runtime claims ownership
- [x] headless launch exits when a tray owner already exists
- [x] tray-vs-tray duplicate launch exits cleanly with the existing already-running behavior
- [x] Step 7: Add unit tests for:
- [x] Windows pystray menu-capable launch
- [x] Linux fallback when `Icon.HAS_MENU` is false
- [x] explicit `--tray-child` failure when tray backend is unavailable
- [x] runtime-owner precedence and duplicate suppression
...
### [x] Checkpoint 3: Split Headless Service Startup from Tray-Login Startup
...
### [x] Checkpoint 4: Enforce Runtime Precedence Between Tray and Headless Startup Paths
...
### [x] Checkpoint 5: Sync Docs, Architecture, Agent Notes, and Full Regression
