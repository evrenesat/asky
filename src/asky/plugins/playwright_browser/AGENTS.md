...
- **Session Persistence**: Automatically manages a persistent browser profile directory (`playwright_profile_<type>`) in the plugin's data directory.
- **Anti-Fingerprinting**:
...
- **Detection**: Some sites may still detect automation. Try using `--browser <url>` to establish a "real" session first.
