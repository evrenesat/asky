...
def has_finding_embeddings_for_model(store: "VectorStore", model: str) -> bool:
    conn = store._get_conn()
    c = conn.cursor()
    if model:
        c.execute(
...
def search_findings(
    ...
    try:
        if not store._has_finding_embeddings_for_model(store.embedding_client.model):
            return []
        ...
        if chroma_ranked_ids:
            ...
            findings_by_id = store._fetch_findings_by_ids(finding_ids)
