...
def mock_daemon_lock_path(tmp_path, monkeypatch):
    monkeypatch.setattr("asky.daemon.runtime_owner.DAEMON_LOCK_PATH", tmp_path / "daemon.lock")
...
def test_run_menubar_app_uses_fake_rumps(monkeypatch, tmp_path):
    ...
    assert app.action_startup.title == "Enable Launch tray at Login"
...
def test_run_menubar_app_rejects_duplicate_instance(monkeypatch, tmp_path):
    ...
    with patch("os.getpid", return_value=12345):
        original_acquire = menubar.acquire_menubar_singleton_lock
        held_lock = original_acquire()
    
    # Second acquisition (collision)
    with patch("os.getpid", return_value=67890):
        with patch("os.kill", return_value=None): # Process 12345 is alive
...
def test_menubar_actions_use_state_aware_labels(monkeypatch, tmp_path):
    ...
    app._refresh_status()
    assert app.action_startup.title == "Disable Launch tray at Login"
...
def test_startup_warnings_displayed_and_cleared(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "asky.daemon.tray_controller.startup_tray.get_status", lambda: _fake_startup()
    )
